...
INFLOW_PAYEE_MAPPING = {
    ...
    'Interests': lambda t: (f'Income:Interests:{t.memo.title().replace(" ","") if t.memo else t.account_name.replace(" ","")}', t.account_name),
    'Item Sale': lambda t: ('Income:ItemSales', t.account_name),
    'Paycheck': lambda t: (f'Income:Salary:{t.memo}', t.account_name),
    ...
    'TFR': lambda t: (f'Income:TFR:{t.memo}', t.account_name),
